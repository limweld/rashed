g#define MOTOR1_ENABLE D0 // Motor 1 enable pin (PWM)
#define MOTOR1_IN1 D1    // Motor 1 direction pin 1
#define MOTOR1_IN2 D2    // Motor 1 direction pin 2
#define MOTOR2_ENABLE D5 // Motor 2 enable pin (PWM)
...
//#define MOTOR2_SPEED_LOW 255  // Lower speed for motor 2
...
IPAddress local_IP(192, 168, 0, 147);
...
void setup() {
  ...
  if (client.connect("ESP8266Client",MQTT_USERNAME,MQTT_PASSWORD)) {
    ...
    mqtt.subscribe("motor_c",controllerMotorASubscribed);
    mqtt.subscribe("motor_d",controllerMotorBSubscribed);
...
void publisher() {
  ...
  if(isMotorARunningTempVar != isMotorARunning){
    mqtt.publish("motor_c","{\"motor3\":{\"status\":"+ isMotorARunning + "}}");
    if(isMotorARunning == "false"){
      mqtt.publish("task","{\"motor3\":{\"job\":\"done\"}}");
    }
    isMotorARunningTempVar = isMotorARunning;
  }
  
  if(isMotorBRunningTempVar != isMotorBRunning){
    mqtt.publish("motor_d","{\"motor4\":{\"status\":"+ isMotorBRunning +"}}");
    if(isMotorBRunning == "false"){
      mqtt.publish("task","{\"motor4\":{\"job\":\"done\"}}");
    }
